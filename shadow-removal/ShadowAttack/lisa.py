...
def model_epoch(
    training_model, data_loader, train=False, optimizer=None, scheduler=None
):
    loss = acc = 0.0
...
def train_model(adv_train=False):
    ...
    training(
        training_model, train_data, train_labels, test_data, test_labels, adv_train
    )
...
def test_model(adv_model=False):
    ...
    trained_model.load_state_dict(
        torch.load(
            f'./model/{"adv_" if adv_model else ""}model_lisa.pth',
