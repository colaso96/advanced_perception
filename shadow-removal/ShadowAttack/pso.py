...
class Particle:
    ...
    def __init__(
        self,
        dim: int = 6,
        coord_min: float = -16.0,
        coord_max: float = 48.0,
        max_speed: float = 1.5,
    ) -> None:
    ...
    def fit_fun(self, position, **parameters):
        ...
        if self.physical:
            img = image_transformation(
                ...
                position,
                self.coord,
                *parameters.get('rand_param'),
                self.pre_process,
            ).to(device)
        else:
            img, shadow_area = draw_shadow(
        ...
        try:
            if self.targeted:
                ...
                success = predict.max(1, keepdim=True)[1] != self.label
